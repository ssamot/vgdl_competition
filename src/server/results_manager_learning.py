...
def process_log(args):
	...
	#One match per game played into the database:
        for game in games:		
            ...
            cur.execute("REPLACE INTO matches (run_id,level_id,game_id,user_id,human_play,actions_file,score,winner,timesteps,match_type) VALUES (%s,%s,%s,%s,0,%s,%s,%s,%s,%s)",[run_id,level_id,game_id,user_id,actions_file,score,winner,timesteps,match_type])
